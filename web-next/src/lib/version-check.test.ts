...
describe("compareBuild", () => {
  ...
  it("commit 缺失时回退版本号比较", () => {
    // 版本一致（含 v 前缀归一化）
    expect(compareBuild("", "unknown", "v0.12.0", "0.12.0")).toBe("match");
    // 版本不同
    expect(compareBuild("", "", "v0.11.0", "v0.12.0")).toBe("mismatch");
  });
  ...
  it("任一侧不可判定（空/dev/unknown）时返回 unknown，不告警", () => {
    ...
    expect(compareBuild("", "bb04581", "", "v0.12.0")).toBe("unknown");
  });
