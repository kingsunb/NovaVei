...
1. **侧边栏布局已是行业标准** —— 4 个参考项目全部使用可折叠侧边栏。现有 NovaVei 的顶部胶囊导航在资源增多后容量不足。
...
其中能力、上下文窗口和显示名称目前是原型的目录预览数据；NovaVei 当前后端 `ChannelModel` 只持久化 `name/source`，
