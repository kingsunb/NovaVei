...
func init() {
	RegisterAfterAutoMigration(Migration{
		Version: 5,
		Up:      migrateV5ChannelToSingleURLAndKey,
	})
}

// migrateV5ChannelToSingleURLAndKey 将多地址、多凭据渠道收敛为单地址、单凭据，并删除旧结构。
func migrateV5ChannelToSingleURLAndKey(db *gorm.DB) error {
	if db == nil {
