...
func init() {
	RegisterBeforeAutoMigration(Migration{
		Version: 3,
		Up:      migrateV3ChannelTypeToAxonhub,
	})
}

// migrateV3ChannelTypeToAxonhub 将旧 transformer 的数字渠道类型迁移为 axonhub/llm 使用的字符串类型。
func migrateV3ChannelTypeToAxonhub(db *gorm.DB) error {
	if db == nil {
