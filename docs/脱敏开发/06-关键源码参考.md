...
> 本文档保留 maskit `engine/transparent.py` 中最值得 NovaVei 移植的关键源码片段，
...
**Go 移植要点**：两个 `map` + `sync.RWMutex`；TTL 用 `time.Time`；NovaVei 可挂到 `X-Session-Id` 会话维度。
...
```python
    def _stream(data: bytes):
        ...
        return text.encode("utf-8") if text else b""  # NovaVei: 返回空列表不写出
```

**Go 移植要点**：NovaVei 已有 SSE 逐 event 解码路径（`relay/protocol.go` + transformer），还原应在解码出的文本增量上操作，而非直接对编码字节做字符串替换。`_sse_text_slots` 的槽位识别逻辑须按三种协议分别适配。
...
**Go 移植要点**：`hashlib.sha256` → `crypto/sha256`；NovaVei `op/error_log.go` 已有 redact 雏形，可强化为同策略。
