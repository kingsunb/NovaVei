...
> **2026-09-13 修复**: 本文列出的高/中/低项已在同日代码中处理（敏感导出改 POST、脱敏会话 TTL、对话页内部接口、JWT 登出轮换、凭据静态加密、CSP、默认绑定等）。下文保留当时的问题描述，便于对照。
...
| 位置 | 值 |
|------|----|
| `main.go` 注释 / README Docker 镜像 | v0.12.0 |
| `CHANGELOG.md` 最新发布 | 0.2.0（2026-08-30） |
| `web-next/package.json` | 0.2.0 |
| `CHANGELOG.md` `[Unreleased]` | 「暂无面向用户的未发布变更」 |

0.12.0 相对 0.2.0 的协议转换、脱敏、对话页、熔断等用户可见变化没有记进 changelog。发版与排障会对不上。
