...
> 现状依据：2026-09-15 对 `web-next/src` 的静态审查。

---

## 1. 背景与目标

当前页面切换的主要问题不是缺少动画，而是切换期间存在多个连续成本：

1. React Router 卸载旧页面。
2. 新页面的动态 chunk 尚未加载完成，进入统一的 `PageSkeleton`。
3. 页面挂载后重新触发各自的 Query 请求。
4. 部分页面（尤其 Logs）在进入后持续触发高频状态更新，造成整页重渲染。
5. 主内容区没有路由级滚动复位，可能继承上一个页面的滚动位置。
...
3. 将昂贵的排序、过滤和运行时状态映射用 `useMemo` 包裹，并保证依赖数组完整。
